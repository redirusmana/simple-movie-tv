...
export const FilterationPage = props => {
    ...
    try {
      ...
      if (pathname === "/movie/popular") {
        if (props.filters && Object.keys(props.filters).length > 0) {
          response = await apiMovieDiscoverGet({
            ...props.filters,
            page: props.pages
          });
        } else {
          response = await apiMoviePopularGet({
            page: props.pages,
        ...
        if (props.filters && Object.keys(props.filters).length > 0) {
          ...
          response = await apiMovieNowPlayingGet({
            page: props.pages,
            api_key: filterState.api_key
          });
        }
        ...
        if (props.filters && Object.keys(props.filters).length > 0) {
          ...
          response = await apiMovieUpcomingGet({
            page: props.pages,
            api_key: filterState.api_key
          });
        }
        ...
        if (props.filters && Object.keys(props.filters).length > 0) {
          ...
          response = await apiMovieTopRatedGet({
            page: props.pages,
            api_key: filterState.api_key
          });
        }
  ...
  const getMovieDiscoverIndex = async pathname => {
    try {
      ...
      if (pathname === "/movie/popular") {
        if (props.filters && Object.keys(props.filters).length > 0) {
          response = await apiMovieDiscoverGet(props.filters);
        } else {
          response = await apiMoviePopularGet({
            api_key: filterState.api_key,
            page: 1
...
const mapStateToProps = state => ({
  dataSource: state.movies.dataSource,
